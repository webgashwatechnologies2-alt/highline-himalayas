...
import himachalimage from "../../../public/images/herobaner/Shimla-During-Monsoon.jpg";
import { allPackages } from "../../lib/packages-data";
import EnquiryModal from "../components/EnquiryModal";

const packages = allPackages.filter(pkg => pkg.destination === "himachal");
...
function BookingForm() {
  ...
  const [destination, setDestination] = useState("Himachal Expeditions");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLead({
        formType: "Himachal Expeditions Booking Form",
        name,
        phone,
        destination,
        departureCity,
  ...
  return (
    <div className="lg:sticky lg:top-24 rounded-3xl overflow-hidden shadow-[0_8px_48px_rgba(21,101,192,0.18)] border border-[#1565C0]/15">
      <div className="relative bg-gradient-to-br from-[#FF8C00] via-[#FF8C00] to-[#FF8C00] px-6 py-5 overflow-hidden">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-amber-300 text-[11px] font-black uppercase tracking-widest">Expedition Booking</span>
          </div>
          <p className="text-white font-bold text-[15px] leading-tight">Plan Your Dream Trip</p>
      ...
      <form onSubmit={handleSubmit} className="bg-white px-5 py-5 space-y-3.5">
        ...
        <div>
          ...
          <div className="relative group">
            <div className="absolute left-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-lg bg-[#1565C0]/10 flex items-center justify-center">
              <MapPin size={12} className="text-[#1565C0]" />
            </div>
            <select
              value={destination}
              onChange={(e) => setDestination(e.target.value)}
              className="w-full pl-10 pr-8 py-2.5 text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200 rounded-xl appearance-none focus:outline-none focus:border-[#1565C0] focus:ring-2 focus:ring-[#1565C0]/10 transition-all cursor-pointer"
            >
              <option>Himachal Expeditions</option>
        ...
        <div>
          ...
          <div className="relative">
            <div className="absolute left-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-lg bg-[#1565C0]/10 flex items-center justify-center">
              <TrendingUp size={12} className="text-[#1565C0]" />
            </div>
            <select
              value={departureCity}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2.5 text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-[#1565C0] focus:ring-2 focus:ring-[#1565C0]/10 transition-all placeholder-slate-400"
              />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              <div className="absolute left-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-lg bg-[#1565C0]/10 flex items-center justify-center">
                <Phone size={12} className="text-[#1565C0]" />
              </div>
              <input
                required
                type="tel"
        ...
        <div>
          ...
          <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 gap-3">
            <div className="w-6 h-6 rounded-lg bg-[#1565C0]/10 flex items-center justify-center shrink-0">
              <Users size={12} className="text-[#1565C0]" />
            </div>
            <button type="button" onClick={() => setTravelers((p) => Math.max(1, p - 1))}
              className="w-7 h-7 rounded-lg bg-white border border-slate-200 text-slate-700 font-black flex items-center justify-center hover:bg-[#1565C0] hover:text-white hover:border-[#1565C0] transition-all text-base leading-none cursor-pointer">
              −
            </button>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-[#FF8C00] to-[#FF8C00] text-white text-[13px] font-black tracking-wide shadow-lg shadow-orange-200 hover:shadow-xl hover:shadow-orange-300 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {loading ? "SENDING..." : "🚀 INITIALIZE BOOKING"}
        </button>

        <a
          href="https://wa.me/918988449735?text=Hello%20Highline%20Himalayas,%20I%20want%20to%20enquire%20about%20Himachal%20packages"
          target="_blank"
          rel="noreferrer"
          className="w-full py-2.5 rounded-2xl border-2 border-[#1565C0] text-[#1565C0] text-[11px] font-black tracking-wide flex items-center justify-center gap-2 hover:bg-[#1565C0] hover:text-white transition-all cursor-pointer"
        >
      ...
      <div className="mt-4 bg-gradient-to-br from-[#1565C0]/5 to-[#FF8C00]/5 border border-[#1565C0]/15 rounded-3xl p-5">
        ...
        <a href="tel:+91898849735" className="block w-full text-center py-2.5 rounded-xl bg-white border-2 border-[#1565C0] text-[#1565C0] text-[12px] font-black hover:bg-[#1565C0] hover:text-white transition-all">
          +91 898849735
        </a>
      </div>
    </div>
            ...
            <nav aria-label="Breadcrumb" className="mb-5 sm:mb-6 flex justify-center">
              <ol className="inline-flex flex-wrap justify-center items-center gap-1.5 sm:gap-2 rounded-2xl sm:rounded-full border border-white/25 bg-white/10 backdrop-blur-md px-3.5 sm:px-5 py-1.5 sm:py-2 text-[10px] sm:text-[11px] font-bold text-white shadow-lg max-w-full text-center">
                ...
                <li className="flex items-center gap-1 text-[#FF8C00]" aria-current="page">
                  <Mountain size={12} /> Himachal Pradesh
                </li>
              </ol>
            </nav>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10">
          <div className="bg-white rounded-3xl shadow-[0_4px_40px_rgba(15,23,42,0.10)] border border-slate-100 overflow-hidden">
            <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-slate-100">
              <StatCard value="500+" label="Happy Travelers" icon={<Users size={22} />} />

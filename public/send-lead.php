...
if ($packageName)
    $rows[] = ['Package Name', $packageName];
if ($packageRoute)
    $rows[] = ['Package Route', $packageRoute];
if ($packagePrice)
    $rows[] = ['Package Price', $packagePrice];
if ($departureCity)
    $rows[] = ['Departure City', $departureCity];
if ($travelDate)
    $rows[] = ['Travel Dates', $travelDate];
if ($travelMonth)
    $rows[] = ['Travel Month / Timeline', $travelMonth];
if ($travelers)
    $rows[] = ['Travelers', $travelers];
if ($travelType)
    $rows[] = ['Trip Type', $travelType];
if ($hotelCategory)
    $rows[] = ['Hotel / Stay Category', $hotelCategory];
if ($whatsappUpdates !== null)
    $rows[] = ['WhatsApp PDF Updates', $whatsappUpdates ? 'Yes' : 'No'];
if ($message)
    $rows[] = ['Customer Message / Notes', $message];
